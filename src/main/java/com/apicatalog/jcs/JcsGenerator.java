...
public final class JcsGenerator extends TreeTraversal implements TreeGenerator {
    ...
    public void node(Object node, TreeAdapter adapter) throws TreeIOException {
        this.entryComparator = TreeComparison.comparingEntryKey(adapter::asString);
    ...
    public void end() throws TreeIOException {
        try {
            ...
            } else if (currentNodeType == NodeType.SEQUENCE) {
                writer.write(']');
            ...
            if (!stack.isEmpty()
                    && stack.peek() instanceof Iterator<?> it
                    && it.hasNext()) {
                writer.write(',');
            }
        } catch (IOException e) {
    ...
    protected void detectNext() throws IOException {
        if ((currentNodeContext == Context.ELEMENT
